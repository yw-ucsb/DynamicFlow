...
data_index = 9
data = torch.load(FILEPATH + 't_whole{}.pt'.format(data_index))
...
def batch_expand(batch):
    stack = torch.stack((batch[:, 0:trajectory_length-1, :], batch[:, 1:trajectory_length, :]), dim=2)
    return stack.reshape(-1, 2, 2)
...
for i in tbar:
    ...
    train_flat = batch_expand(batch)
    # print(batch.shape)
    optimizer.zero_grad()
    train_log_prob = -flow.log_prob(
        inputs=train_flat[:, 0, :], context=train_flat[:, 1, :]
    ).reshape(batch_size, trajectory_length - 1)
    loss = torch.sum(train_log_prob, dim=1).mean()
    ...
    if (i + 1) % val_interval == 0:
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                ...
                val_flat = batch_expand(val_batch)
                val_log_prob = -flow.log_prob(
                    inputs=val_flat[:, 0, :], context=val_flat[:, 1, :]
                ).reshape(batch_size, trajectory_length - 1)
                log_density_val = torch.sum(val_log_prob, dim=1).mean()
        ...
        if running_val_log_density < best_val_score:
            ...
            torch.save(flow.state_dict(), FILEPATH + 'nf_t_whole{}_bs{}_hd{}_nl{}.t'.format(data_index, batch_size, dim_hidden, num_layers))
...
flow.load_state_dict(torch.load(FILEPATH + 'nf_t_whole{}_bs{}_hd{}_nl{}.t'.format(data_index, batch_size, dim_hidden, num_layers)))
flow.eval()
...
plt.title('nf_t_whole{}_bs{}_hd{}_nl{}.t'.format(data_index, batch_size, dim_hidden, num_layers))
...
# rangee = 4
# bias = 0.5
# x = torch.linspace(-rangee + bias, rangee + bias, step)
# y = torch.linspace(-rangee + bias, rangee + bias, step)
...
# plt.title('Density, log scaled')
# # plt.contourf(X.numpy(), Y.numpy(), Z.numpy(), cmap='summer')
# plt.contourf(X.numpy(), Y.numpy(), Z.numpy(), cmap='summer', norm=colors.LogNorm())
#
#
# plt.subplot(1, 2, 2)
# sample_context = torch.zeros([1, 2]) + bias
# s = flow._sample(100000, sample_context.to(device)).detach().numpy()
# s_mean = torch.tensor([s[0, :, 0].mean(), s[0, :, 1].mean()])
# # plt.title('Mean: [{:3f}, {:3f}]'.format(s_mean.numpy()[0], s_mean.numpy()[1]))
# plt.title('Samples')
#
# plt.scatter(s[0, :, 0], s[0, :, 1], alpha=0.02)
# plt.show()