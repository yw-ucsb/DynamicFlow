...
def visualize(true_y, pred_y, odefunc, itr):
    ...
    ax_traj.plot(t.cpu().numpy(), true_y.cpu().numpy()[:, 0, 0], t.cpu().numpy(),
                 true_y.cpu().numpy()[:, 0, 1], 'g-', label='true')
    ax_traj.plot(t.cpu().numpy(), pred_y.cpu().numpy()[:, 0, 0], '--', t.cpu().numpy(),
                 pred_y.cpu().numpy()[:, 0, 1], 'b--', label='pred')
    ...
    figure.tight_layout()
    plt.savefig(args.FILEPATH + 'demo_itr{:03d}.png'.format(itr))
    # plt.draw()
    # plt.pause(0.001)
