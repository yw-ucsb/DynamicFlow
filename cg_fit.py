...
CG = ConditionalDiagonalNormal(shape=[dim],
...
optimizer = optim.Adam(CG.parameters(), lr=lr)
...
#     # print(batch.shape)
#     optimizer.zero_grad()
#     loss = -CG.log_prob(inputs=batch[:, 0, :], context=batch[:, 1, :]).mean()
...
#                 log_density_val = -CG.log_prob(inputs=val_inputs.to(device).detach(), context=val_context.to(device).detach())
...
#             torch.save(CG.state_dict(), FILEPATH + 'cg_model_t{}.t'.format(data_index))
...
CG.load_state_dict(torch.load(FILEPATH + 'cg_model_t{}.t'.format(data_index)))
CG.eval()
...
#     samples = CG._sample(5000, context.to(device)).detach().cpu()
#     # for j in range(bs):
...
for i in range(t_length):
    trajectory0[i] = context.detach().cpu()
    s = CG._sample(1, context.to(device)).squeeze(dim=1)
    context = s
cov = cov.transpose(dim0=0, dim1=1)
mean_tr = torch.mean(trajectory0, dim=1)
