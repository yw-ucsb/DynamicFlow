...
def cg_encoder(context):
    bs = context.shape[0]
    A = torch.eye(dim) * 1.005
    # A = torch.rand([dim, dim])
    b = torch.zeros(dim) + 0.005
    mean = context @ A + b
    logcov = torch.zeros([bs, dim]) - 3
    return torch.cat((mean, logcov), dim=1)

base_dist = ConditionalDiagonalNormal(shape=[dim],
                               context_encoder=cg_encoder
                               )
...
for i in tbar:
    ...
    if (i + 1) % val_interval == 0:
        ...
        if running_val_log_density < best_val_score:
            ...
            torch.save(flow.state_dict(), FILEPATH + 'truecgnf_t{}_hd{}_nl{}.t'.format(data_index, dim_hidden, num_layers))
...
flow.load_state_dict(torch.load(FILEPATH + 'truecgnf_t{}_hd{}_nl{}.t'.format(data_index, dim_hidden, num_layers)))
flow.eval()
...
plt.title('truecgnf_t{}_bs{}_hd{}_nl{}.t'.format(data_index, batch_size, dim_hidden, num_layers))
