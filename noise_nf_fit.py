...
# FILEPATH = 'D:\/Study_Files\/UCSB\/Projects\/RL\/NF\/trajectory_data\/'
FILEPATH = 'D:\/Coding\/RL\/NF\/trajectory_data\/'
...
for i in tbar:
    ...
    if (i + 1) % val_interval == 0:
        ...
        if running_val_log_density < best_val_score:
            ...
            torch.save(flow.state_dict(), FILEPATH + 'noisenf_nf_model_t{}.t'.format(data_index))
            torch.save(RESNET.state_dict(), FILEPATH + 'noisenf_res_model_t{}.t'.format(data_index))
...
flow.load_state_dict(torch.load(FILEPATH + 'noisenf_nf_model_t{}.t'.format(data_index)))
flow.eval()
RESNET.load_state_dict(torch.load(FILEPATH + 'noisenf_res_model_t{}.t'.format(data_index)))
RESNET.eval()
...
t_length = 100
trajectory_s = torch.zeros(t_length, bs, dim)
trajectory_p = torch.zeros(t_length, bs, dim)
cov = torch.zeros(t_length, bs, dim, dim)
mean = torch.zeros(t_length, bs, dim)
for i in range(t_length):
    trajectory_s[i] = context.detach().cpu()
    trajectory_p[i] = context.detach().cpu()
    p = RESNET(context)
    s = flow._sample(1, p.to(device)).squeeze(dim=1)
    # context = p
    context = s
mean_tr_s = torch.mean(trajectory_s, dim=1)
...
# for i in range(bs):
#     plt.scatter(trajectory_s[:, i, 0], trajectory_s[:, i, 1])
plt.scatter(mean_tr_s[:, 0], mean_tr_s[:, 1])
plt.show()